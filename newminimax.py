...
class GameContent_min(GameContent):
    ...
    @staticmethod 
    def minimax(state, depth, maximizing_player, indent=0): 
    ...
    def get_possible_moves(state):
        moves = []
        # Find all moves that form pairs [0, 0] or [1, 1]
        for i in range(len(state) - 1):
            if state[i] == state[i + 1]:
                moves.append((i, i + 1))
        return moves
    ...
    def get_possible_moves(state):
        ...
        # If no moves form pairs [0, 0] or [1, 1], find all adjacent indices pairs
        if not moves:
            for i in range(len(state) - 1):
                moves.append((i, i + 1))
    ...
    def get_best_move(state, maximizing_player=True, depth=4, indent=0): 
        ...
        for move in possible_moves:
            ...
            if (maximizing_player and eval > max_eval) or (not maximizing_player and eval < max_eval):
                max_eval = eval
                best_moves = [move]  # Update with only the current move (same as before)
            elif eval == max_eval:
                best_moves.append(move)  # Append the current move to best_moves if its evaluation is equal to max_eval
        ...
        return random.choice(best_moves) if best_moves else GameContent_min.get_random_move(state)
    @staticmethod
    def evaluate(state):
        ai_score = 0
        for i in range(len(state) - 1):
